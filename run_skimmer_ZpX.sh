echo -e "Sample,Had 2e,Had 1mu,Passed Trigger,Had No b Jets,Found Good 2e+1mu,Found Z Candidate,Overall Efficiency" > Tables/cut_list.txt
echo -e "Sample,Failed Good 2e+1mu,Tight Id,e pT,e Iso,e SIP,e dxyz,mu pT" > Tables/GoodMu_list.txt
...
for i in ${!data_list[@]}; do
    python skimmer_ZpX.py ${data_list[i]}
done

for i in ${!bkg_list[@]}; do
	python skimmer_ZpX.py ${bkg_list[i]}
done

python3 Tables/pretty_table.py > Tables/Efficiency_table.txt
cat Tables/Efficiency_table.txt

source removeDuplicates/removeDuplicates_ZpX.sh
